...
#   model_path:训练出的自己的模型参数权值所存放的路径
#   classes_path:存放着自己要识别的类别
#--------------------------------------------#
class YOLO(object):
    _defaults = {
        "model_path"        : 'logs/1.pth',
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image,isRetImg=False):
        #---------------------------------------------------------#
        ...
        font = ImageFont.truetype(font='yolov4tiny/model_data/simhei.ttf',size=np.floor(3e-2 * np.shape(image)[1] + 0.5).astype('int32'))
        ...
        for i, c in enumerate(top_label):
            ...
            del draw
        return {'labels':top_label,'img':image}
